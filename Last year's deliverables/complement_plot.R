...
# データの読み込み
ds_b <- read_csv("modified_df_2017.csv", col_names = F, skip = 1)
ds_a <- read_csv("analysis_df_2017.csv", col_names = F, skip = 1)
...
### 外れ値の箇所を取り出す ###
str_i <- readline("列番号を入力してください(2~56): ")
i <- as.numeric(str_i)
rule_i <- paste0("z", i)
xi <- paste0("X", i)

ds_b2 <- ds_b %>% mutate(days = substr(ds_b$X1, 1, 10))
id_i <- num_outliers %>% filter(rule == rule_i) %>% select(id)
day_i <- ds_b2[id_i$id,] %>% select(days)
print(day_i)
str_j <- readline("プリント日付番号のうちいずれかを入力してください(ex. 1): ")
str_k <- readline("電力消費量の上限値を入力してください(ex. 300): ")
j <- as.numeric(str_j)
k <- as.numeric(str_k)
dayi <- ds_b2 %>% filter(days == day_i$days[j]) %>% select(X1, xi) %>% 
  group_by(X1) %>% summarise_each(funs(sum))


### 補完前のグラフ ###
x1 <- "X1"
pic_b <- dayi %>% ggplot(aes_string(x=x1, y=xi)) + geom_line() + geom_point() +
  xlab("時刻") + ylab("消費電力量[kWh]") + ylim(0, k) +
  theme(plot.title = element_text(size = 40),
        axis.title.x = element_text(size = 20), axis.text.x = element_text(size = 15, face = "bold"),
        axis.title.y = element_text(size = 20), axis.text.y = element_text(size = 15, face = "bold"))
print(pic_b)  

### 補完後のグラフ ###
ds_a2 <- ds_a %>% mutate(days = substr(ds_a$X1, 1, 10))
c_i <- ds_a2[id_i$id,] %>% select(days)
c_dayi <- ds_a2 %>% filter(days == c_i$days[j]) %>% select(X1, xi)

pic_a <- c_dayi %>% 
  ggplot(aes_string(x=x1, y=xi)) + geom_line() + geom_point() +
  xlab("時刻") + ylab("消費電力量[kWh]") + ylim(0, k) +
  theme(plot.title = element_text(size = 40),
        axis.title.x = element_text(size = 20), axis.text.x = element_text(size = 15, face = "bold"),
        axis.title.y = element_text(size = 20), axis.text.y = element_text(size = 15, face = "bold"))
...
grid.arrange(pic_b, pic_a, nrow = 2)
