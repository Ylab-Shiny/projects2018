...
# データを取得
data <- readr::read_csv(paste0(dir2012, filenames[1]), col_names = T)
data <- data %>% mutate(
  label = data$日時,
  date = substr(data$日時, 1, 10)
)
...
# labelの型を文字列に変換
dep_2012$label <- as.character(dep_2012$label)

### 保存するデータセットは2012年度の5分間隔の平均消費電力[kW] ###
